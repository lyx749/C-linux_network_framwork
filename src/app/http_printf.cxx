...
static char *http_sprintf_num(char *buff, char *last, uint64_t ui64, char zero, uintptr_t hexadecimal, uintptr_t width);
char *http_snprintf(char *buf, size_t max, const char *fmt, ...) // 对http_vslprintf的封装，提供了缓冲区结束位置，更加安全
{
    va_list args;
    char *p = NULL;
    va_start(args, fmt); // 函数可变参数是一个从左往右的一个传参，让args指向开始的参数
...
char *http_vslprintf(char *buf, char *last, const char *fmt, va_list args)
{
    ...
    while (*fmt && buf < last)
    {
        ...
        if (*fmt == '%')
        {
            zero = (char)((*++fmt == '0') ? '0' : ' '); // http_sprintf_num() 格式化输出数字长度不足时的填充字符

            sign = 1;       // 是否显示有符号数，默认1，有符号，如果是 %u 无符号
            width = 0;      // http_sprintf_num() 格式化输出数字的长度
            hex = 0;        // 是否以16进制显示
...
static char *http_sprintf_num(char *buf, char *last, uint64_t ui64, char zero, uintptr_t hexadecimal, uintptr_t width)
{
    char *p, temp[HTTP_INT64_LEN + 1]; // sizeof("-9223372036854775808") - 1 = 20
    size_t len;
    uint32_t ui32;
    ...
    p = temp + HTTP_INT64_LEN; // p 指向数组最后一个元素位置

    if (hexadecimal == 0)
    { // 非16进制显示
        if (ui64 <= (uint64_t)HTTP_MAX_UINT32_VALUE)
        {
            ui32 = (uint32_t)ui64;
            do
            { // uin32 = 7654321 --> temp = [            7654321 ]
                *--p = (char)(ui32 % 10 + '0');
            } while (ui32 /= 10);
        }
        else
    ...
    len = (temp +HTTP_INT64_LEN) - p; // 处理后数字字符串长度
    ...
    len = (temp + HTTP_INT64_LEN) - p; // 还原 len，准备拷贝到 buf 中

    if ((buf + len) >= last)
    {                     // buf 不足以拷贝整个数值字符串
        len = last - buf; // 有多少容量便拷贝多少
    }
    return http_memcpy(buf, p, len); // 返回 buf
}
